...
async def load_reload(event):
    ...
    try:
        ...
        msg = await event.respond(f"Successfully reloaded plugin {shortname}")
...
async def remove(event):
    ...
    shortname = event.pattern_match["shortname"]
    if shortname == "_core":
        msg = await event.respond(f"Core plugin {shortname} can not be removed.")
    elif shortname in borg._plugins:  # pylint:disable=E0602
        borg.remove_plugin(shortname)  # pylint:disable=E0602
        msg = await event.respond(f"Unloaded plugin {shortname}")
    else:
...
@borg.on(util.admin_cmd(pattern="plugin (?P<shortname>\w+)$"))  # pylint:disable=E0602
    ...
    if event.reply_to_msg_id:
        try:
            ...
            if "(" not in downloaded_file_name:
                ...
                await event.edit("Loaded Plugin `{}`".format(os.path.basename(downloaded_file_name)))
            else:
                os.remove(downloaded_file_name)
                await event.edit("Plugin already exists!")
        except Exception as e:  # pylint:disable=C0103,W0703
