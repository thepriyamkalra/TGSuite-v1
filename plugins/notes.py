...
from sqlhelpers.notes_sql import get_notes, rm_note, add_note, rm_all_notes
import asyncio
import time
from userbot import syntax


@bot.on(command(pattern="notes ?(.*)"))
async def _(svd):
    if svd.fwd_from:
        return
...
@bot.on(command(pattern="clearall ?(.*)"))
async def _(prg):
    if prg.fwd_from:
        return
