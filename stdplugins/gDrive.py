...
async def _(event):
    ...
    if required_file_name:
        ...
        try:
            ...
            await event.client.send_message(int(Config.PRIVATE_GROUP_ID), "Please add Config AUTH_TOKEN_DATA with the following as the value:\n\n`" + token_file_data + "`")
...
async def _(event):
    ...
    if Config.PRIVATE_GROUP_ID is None:
        await event.edit("Please set the required environment Configiable `PRIVATE_GROUP_ID` for this plugin to work")
        return
    input_str = event.pattern_match.group(1)
...
async def create_token_file(token_file, event):
    ...
    async with bot.conversation(int(Config.PRIVATE_GROUP_ID)) as conv:
        ...
        response = conv.wait_event(events.NewMessage(
            outgoing=True,
            chats=int(Config.PRIVATE_GROUP_ID)
        ))
